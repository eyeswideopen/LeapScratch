...
class LeapController(Leap.Listener):
    def __init__(self,notifyFunction,calcScale):
    ...
    def start(self):
        ...
        self.notifyFunction(False,False,(0,0),self.crossfade,self.volume,1)
    ...
    def getScale(self):
        ...
        if len(path)>=1:
            ...
            breaking=False
            if coordinate and coordinate.y < 150 and coordinate.x>0:
                scale= self.calculateScale(self.prevCoordinate.x,self.prevCoordinate.z,coordinate.x,coordinate.z)
            elif coordinate and coordinate.y<250 and coordinate.x>0:
                scale=(coordinate.y-150)/100
                breaking=True
            else:
                scale= 1
            ...
            self.notifyFunction(breaking, True if scale!=1 and not breaking else False,coordinate,self.crossfade,self.volume,scale)
