...
class NewFileHandler:
    ...
    def readFromFile(self, frames, reverse):
        ...
        #interpoaltion buffer hack
        # frames = frames + 3

        #3 for each channel in int16 => 6 * 1 byte. Used for 6 point interpolation.
    ...
    def resample(self, inData, length, interpolationBufferTail):
        ...
        ##
        ## test the waveInterpoaltor
        ##

        # testparams = numpy.linspace(1000,5000, num=6).astype(numpy.int16)
        # print testparams
        # erg = waveInterpolator(0.1, testparams)
        # print int(erg)
        #scheint zu gehen
        ...
        # interpolationBufferHead = inData[len(inData)-3:]



        #indices
        interpolationIndices = numpy.linspace(0, len(inData)-4, num=length) #-4 because we use the last 3 values as phantom values
        ...
        ##
        ##scipy piecwise cubic hermite interpolation: way to slow!
        ##

        # f = interpolate.PchipInterpolator(x, numpyData)
        #
        # for i in range(len(interpolationIndices)):
        #     interpolationIndices[i] = f(interpolationIndices[i])
        #
        # interpolationIndices = interpolationIndices.tostring()
        # return interpolationIndices


        ##
        ##this is the more or less working linear interpolator
        ##

        return numpy.interp(
            numpy.linspace(0.0, 1.0, length, endpoint=False),
            numpy.linspace(0.0, 1.0, len(inData), endpoint=False),
            inData,
        )
